...
export default async function handler(req, res) {
  ...
  if (!userId) return res.status(401).json({ error: 'Debes iniciar sesion.' });
  ...
  try {
    ...
    const tightSystem = (systemPrompt || '') + `\n\nREGLA ABSOLUTA: Responde SOLO con JSON valido. Sin texto antes ni despues. Sin saltos de linea dentro de strings. Usa comillas simples dentro de valores si necesitas citar. No uses caracteres especiales dentro de strings.`;
    ...
    // Try to parse — if it fails, do deeper cleanup
    let parsed;
    try {
      ...
      try {
        ...
        // Last resort: return error with raw text for debugging
        return res.status(500).json({ 
          error: 'JSON parse error: ' + e2.message.substring(0, 100)
        });
      }
    }
    ...
    // Send parsed object directly — no more JSON string issues on the client

export default async function handler(req, res) {
  ...
  const { dbId, oferta } = req.body;
  const token = process.env.NOTION_TOKEN;
  console.log('Token present:', !!token, 'Token prefix:', token ? token.substring(0, 10) : 'none');
  console.log('dbId:', dbId);
  if (!token) return res.status(400).json({ error: 'NOTION_TOKEN env var not set' });
  if (!dbId || !oferta) return res.status(400).json({ error: 'Missing fields' });
